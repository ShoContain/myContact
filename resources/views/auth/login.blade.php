...
<div class="mx-auto h-full flex justify-center items-center bg-gray-300">
    <div class="w-96 bg-blue-900 rounded-lg shadow-xl p-6">
      ...
      <h2 class="text-blue-300 text-2xl">ログインしてください</h2>

                    <form method="POST" action="{{ route('login') }}" class="pt-10">
                        @csrf

                        <div class="relative">
                            <label for="email" class="uppercase text-blue-500 text-xs font-bold absolute pl-3 pt-1">メール</label>
                            <div>
                                <input id="email" type="email" class="pt-4 w-full rounded p-3 bg-blue-800 text-gray-100 outline-none focus:bg-blue-700" name="email" value="{{ old('email') }}" autocomplete="email" autofocus placeholder="your@email.com">
                                @error('email')
                                    <span class="text-red-400 text-sm pl-2 " role="alert">
                        ...
                        <div class="relative pt-3">
                            <label for="password" class="uppercase text-blue-500 text-xs font-bold absolute pl-3 pt-1 ">パスワード</label>

                            <div>
                                <input id="password" type="password" class="pt-4 w-full rounded p-3 bg-blue-800 text-gray-100 outline-none focus:bg-blue-700" name="password" value="{{ old('password')}}" autocomplete="current-password" autofocus placeholder="Password">
                                @error('password')
                                    <span class="text-red-400 text-sm pl-2 " role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                        ...
                        <div class="pt-6">
                          <button type="submit" class="uppercase rounded text-gray-300 bg-gray-500 w-full font-bold text-blue-900 px-3 py-2 text-left">
                              login
                          </button>
                        </div>
