...
<div class="bg-gray-300 mx-auto h-full flex justify-center items-center">
        <div class="w-96 p-6 rounded-lg bg-blue-900 shadow-xl">
            ...
            <h2 class="text-base text-blue-300">パスワードの再発行にはメールアドレスでの認証が必要です</h2>

            <form method="POST" action="{{ route('password.email') }}" class="pt-8">
                @csrf

                <div class="relative">
                    <label for="email" class="text-blue-500 text-xs font-bold pl-3 pt-2 absolute uppercase">メールアドレス</label>

                    <input id="email" type="email" class="w-full p-3 pt-4 rounded focus:outline-none focus:bg-blue-700 bg-blue-800 text-gray-100 @error('email') border border-red-600 @enderror" name="email" value="{{ old('email') }}" required autocomplete="email" autofocus placeholder="your@email.com">
                ...
                <div class="pt-6">
                    <button type="submit" class="w-full rounded py-2 px-3 uppercase text-left bg-gray-400 text-blue-800 text-gray-100 font-bold">送信</button>
                </div>
